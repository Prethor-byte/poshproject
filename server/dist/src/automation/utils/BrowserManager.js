...
class BrowserManager {
    constructor() {
        this.sessionLocks = new Map(); // For race condition prevention
    ...
    async monitorSessions() {
        for (const [userId, session] of this.sessions.entries()) {
            try {
                ...
                logger_1.logger.error('Session monitoring failed', {
                    error,
                    userId,
    ...
    async recoverSession(userId) {
        ...
        if (session.health.recoveryAttempts >= 3) {
            logger_1.logger.error('Max recovery attempts reached, session will be terminated', {
                userId,
                sessionId: session.profile.id
            });
            await this.closeSession(userId);
        ...
        try {
            ...
            // Create new session
            const { browser, context } = await this.retryManager.executeWithBackoff(async () => {
            ...
            logger_1.logger.info('Session recovered successfully', {
                userId,
                sessionId: session.profile.id,
    ...
    async checkSessionHealth(session) {
        try {
            ...
            const errorType = error instanceof errors_1.AutomationError ? error.type : errors_1.ErrorType.UNKNOWN;
            const recoverable = error instanceof errors_1.AutomationError ? error.recoverable : true;
    ...
    async updateSessionMetrics(userId, metrics) {
        ...
        if (sessionMetrics.totalOperations === 1) {
            ...
            sessionMetrics.averageResponseTime = ((sessionMetrics.averageResponseTime * (sessionMetrics.totalOperations - 1) + operationDuration) /
                sessionMetrics.totalOperations);
        }
        this.updateHealthStatus(session);
    }
    updateHealthStatus(session) {
        ...
        const hasRecentErrors = errors.some(error => Date.now() - error.timestamp.getTime() < 5 * 60 * 1000 // 5 minutes
        );
    ...
    async createSession(profile, userId) {
        ...
        const lockPromise = (async () => {
            if (!await this.rateLimiter.acquireToken(userId)) {
                ...
                throw new errors_1.AutomationError(`Rate limit exceeded. Try again after ${info.nextResetTime.toISOString()}`, errors_1.ErrorType.RATE_LIMIT, true);
            }
            try {
                const browser = await playwright_1.chromium.launch({
                    headless: process.env.NODE_ENV === 'production',
                ...
                logger_1.logger.error('Failed to create browser session', { error });
                // Always throw AutomationError for test predictability
                throw new errors_1.AutomationError('Browser session creation failed', errors_1.ErrorType.SETUP_FAILED, true);
            }
            finally {
    ...
    async closeSession(userId) {
        ...
        if (session) {
            try {
                ...
                logger_1.logger.error('Failed to close browser session', { error });
            }
            finally {
    ...
    async closeAllSessions() {
        this.stopMonitoring();
        const closePromises = Array.from(this.sessions.keys()).map(userId => this.closeSession(userId));
